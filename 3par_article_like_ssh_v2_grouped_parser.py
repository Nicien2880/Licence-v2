...
def is_pure_number_token(token: str) -> bool:
    return re.fullmatch(r"[-+]?\d+(?:\.\d+)?", str(token).strip()) is not None


def numeric_tail(tokens: List[str]) -> Tuple[List[str], List[str]]:
    """Return prefix tokens and right-side contiguous numeric tail."""
    tail: List[str] = []
    i = len(tokens) - 1
    while i >= 0 and is_pure_number_token(tokens[i]):
        tail.insert(0, tokens[i])
        i -= 1
    return tokens[:i + 1], tail


def parse_grouped_stat_summary(text: str, label: str) -> Dict[str, Any]:
    """Parse 3PAR stat* grouped output by numeric tail positions.

    Typical output has multi-line headers, for example:
      r/w per second: Cur Avg Max
      KBytes per sec: Cur Avg Max
      Svt ms: Cur Avg Max

    The generic table parser cannot map duplicate Cur/Avg/Max headers, so this
    parser maps by numeric column positions.
    """
    ...
    for raw_line in text.splitlines():
        ...
        if any(h in low for h in ["date", "time", "cur", "avg", "max", "kbytes", "second", "svt", "iosize", "idle", "qlen"]):
            continue
        if set(line) <= {"-", "=", "#", " "}:
            continue
        ...
        # Need at least r/w cur/avg/max + KB cur/avg/max + Svt cur/avg/max.
        if len(tail) < 9:
            continue
        detected += 1

        prefix_norm = [str(x).strip().lower() for x in prefix]
        # Avoid double-counting summary rows named total.
        if "total" in prefix_norm and len(prefix_norm) <= 3:
            continue
        ...
        for token in reversed(prefix):
            ...
            if c in ("read", "write"):
                direction = c
                break
            if t in ("total", "tot", "r/w", "rw"):
                direction = "total"
                break

        nums = [fnum(x) or 0.0 for x in tail]
        vals = {
            "rw_cur": nums[0], "rw_avg": nums[1], "rw_max": nums[2],
            "kb_cur": nums[3], "kb_avg": nums[4], "kb_max": nums[5],
        }
        # In this article-style output RWCur equals IO per second.
        vals["io_cur"] = vals["rw_cur"]
        vals["io_avg"] = vals["rw_avg"]
        vals["io_max"] = vals["rw_max"]
        svt_cur, svt_avg, svt_max = nums[6], nums[7], nums[8]

        # After IOSize columns, some stat commands include QLen and Idle%.
        if len(nums) >= 15:
            vals["qlen_cur"], vals["qlen_avg"], vals["qlen_max"] = nums[12], nums[13], nums[14]
        if len(nums) >= 18:
            # Often the last group is Idle%; convert to Busy% for rough aggregate use.
            vals["busy_cur"] = max(0.0, min(100.0, 100.0 - nums[15]))
            vals["busy_avg"] = max(0.0, min(100.0, 100.0 - nums[16]))
            vals["busy_max"] = max(0.0, min(100.0, 100.0 - nums[17]))

        used += 1
        for k, v in vals.items():
            add_metric(out[direction], k, v)

        den_cur = vals["io_cur"] if vals["io_cur"] not in (None, 0) else 1.0
        den_avg = vals["io_avg"] if vals["io_avg"] not in (None, 0) else 1.0
        svt_weight[direction]["cur_num"] += svt_cur * den_cur
        svt_weight[direction]["cur_den"] += den_cur
        svt_weight[direction]["avg_num"] += svt_avg * den_avg
        svt_weight[direction]["avg_den"] += den_avg
    ...
    return {
        ...
        "rows_used": used,
        "label": label,
        "parser_mode": "grouped_numeric_tail",
    }
...
def parse_rw_summary(text: str, label: str) -> Dict[str, Any]:
    ...
    grouped = parse_grouped_stat_summary(text, label)
    if grouped.get("rows_used", 0) > 0:
        return grouped
...
def main() -> int:
    ...
    result: Dict[str, Any] = {
        "status": 0,
        "collector": "3par_article_like_v2.py",
